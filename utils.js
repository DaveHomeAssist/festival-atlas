(function attachUtilsModule(global) {
  ...
  function buildUsBasemap(width, height, options) {
    ...
    return [
      '<g class="atlas-map-basemap" aria-hidden="true">',
      '  <polygon class="atlas-map-land" points="' + outline + '" fill="' + landFill + '" stroke="' + coastStroke + '" stroke-width="1.4" stroke-linejoin="round"></polygon>',
      regionLines.map(function mapRegion(line) {
        return '  <polyline class="atlas-map-region" points="' + line + '" fill="none" stroke="' + regionStroke + '" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"></polyline>';
      }).join(""),
      '</g>'
    ].join("");
  }
